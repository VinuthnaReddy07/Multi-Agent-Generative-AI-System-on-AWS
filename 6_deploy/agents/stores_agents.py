...
MENU_KNOWLEDGE_BASE_ID = os.environ["STORES_KNOWLEDGE_BASE_ID"]
...
host = next(
    ...
    if c['DBClusterIdentifier'].startswith('path-to-production-gen-ai-application-p2pdbcluster')
)
print(f"Aurora endpoint resolved: {host}")

print("Fetching DB credentials from Secrets Manager...")
secrets_client = boto3.client('secretsmanager')
secret_value = secrets_client.get_secret_value(SecretId='p2p-aurora-db-credentials')
